...
Resources:
- https://privy.io`;return this._preparedMessage=f,{nonce:e,message:f}}constructor(a,b){this._wallet=void 0,this._privyInternal=a,this._embedded=b}}let bk={path:"/api/v1/siws/init",method:"POST"},bl={path:"/api/v1/siws/authenticate",method:"POST"},bm={path:"/api/v1/siws/link",method:"POST"},bn={path:"/api/v1/siws/unlink",method:"POST"};class bo{async unlink({address:a}){await this._privyInternal.fetch(bn,{body:{address:a}});let b=await this._privyInternal.refreshSession();return{user:b.user,identity_token:b.identity_token}}async link({message:a,signature:b,walletClientType:c,connectorType:d}){await this._privyInternal.fetch(bm,{body:{message:a,signature:b,walletClientType:c??null,connectorType:d??null}});let e=await this._privyInternal.refreshSession();return{user:e.user,identity_token:e.identity_token}}async login({mode:a,message:b,signature:c,walletClientType:d,connectorType:e,opts:f}){let g=await this._privyInternal.fetch(bl,{body:{signature:c,message:b,walletClientType:d??null,connectorType:e??null,mode:a}});await this._privyInternal.session.updateWithTokensResponse(g);let h=await aA(this._embedded,g,f?.embedded);return this._privyInternal.callbacks?.setUser?.(h.user),h}async fetchNonce({address:a}){let{nonce:b}=await this._privyInternal.fetch(bk,{body:{address:a}});return{nonce:b}}constructor(a,b){this._privyInternal=a,this._embedded=b}}class bp{async link(a,b,c,d){await this._privyInternal.fetch(bh,{body:{message:a,signature:b,smart_wallet_type:c,smart_wallet_version:d}});let e=await this._privyInternal.refreshSession();return{user:e.user,identity_token:e.identity_token}}async init(a){var b;let{nonce:c}=await this._privyInternal.fetch(be,{body:{address:a.address}});return{nonce:c,message:`${(b={chainId:a.chainId.toString().replace("eip155:",""),address:a.address,issuedAt:(new Date).toISOString(),statement:"By signing, you are proving you own this wallet and logging in. This does not initiate a transaction or cost any fees.",domain:"privy.io",uri:"https://auth.privy.io",nonce:c}).domain} wants you to sign in with your Ethereum account:
${b.address}
