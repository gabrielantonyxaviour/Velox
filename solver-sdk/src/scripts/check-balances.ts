...
async function main() {
  ...
  try {
    ...
    console.log('MOVE (native): Error fetching -', (error as Error).message);
  }
  ...
  try {
    ...
    console.log('tUSDC: Error -', (error as Error).message);
  }
  ...
  try {
    const result = await aptos.view({
      payload: {
        function: `${VELOX_ADDRESS}::test_tokens::get_token_b_balance` as `${string}::${string}::${string}`,
        typeArguments: [],
        functionArguments: [VELOX_ADDRESS, SOLVER_ADDRESS],
      },
    });
    ...
    console.log('tMOVE: Error -', (error as Error).message);
  }
